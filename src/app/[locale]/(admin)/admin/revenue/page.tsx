    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-auto p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <Card>
            <h3 className="text-sm font-semibold text-gray-900 mb-4">{t('subscriptionBreakdown')}</h3>
            <div className="h-64">
              {pieData.length > 0 ? (
          ...
          {/* Monthly Growth */}
          <Card>
            <h3 className="text-sm font-semibold text-gray-900 mb-4">{t('monthlyGrowth')}</h3>
            <div className="h-64">
              {metrics?.monthly_growth && metrics.monthly_growth.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={metrics.monthly_growth}>
                    ...
                    <Legend />
                    <Bar dataKey="new_orgs" fill="#9ca3af" radius={[2, 2, 0, 0]} name={t('newOrgs')} />
                    <Bar dataKey="new_paying" fill="#10b981" radius={[2, 2, 0, 0]} name={t('newPaying')} />
                  </BarChart>
                </ResponsiveContainer>
              ) : (
                <div className="flex h-full items-center justify-center text-gray-400 text-sm">
        ...
        {/* Revenue Projection */}
        <Card>
          <h3 className="text-sm font-semibold text-gray-900 mb-4">{t('revenueSnapshot')}</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-wide">{t('monthlyRevenue')}</p>
              <p className="mt-1 text-xl font-bold text-gray-900">${Number(metrics?.mrr ?? 0).toFixed(0)}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-wide">{t('projectedAnnual')}</p>
              <p className="mt-1 text-xl font-bold text-gray-900">${Number(metrics?.arr ?? 0).toFixed(0)}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-wide">{t('avgRevenuePerOrg')}</p>
            ...
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-wide">{t('totalPaying')}</p>
              <p className="mt-1 text-xl font-bold text-gray-900">
