...
export default function DisplayPage() {
  ...
  if (!org) {
    return (
      <div className="flex h-screen items-center justify-center bg-gray-950">
        <p className="text-2xl text-gray-400">Organization not found</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen flex-col bg-gray-950 text-white overflow-hidden select-none">
        ...
        {queues.map((queue) => {
          ...
          return (
            ...
            >
              ...
              {/* Now Serving label */}
              <p className="text-lg text-gray-500 mb-2">Now Serving</p>
      ...
      <footer className="flex items-center justify-between px-10 py-4 border-t border-gray-800">
        <p className="text-sm text-gray-600">QueueFlow by CareLabs Sweden</p>
        <p className="text-sm text-gray-600">
          Join the queue at <span className="text-gray-400 font-medium">{typeof window !== 'undefined' ? window.location.origin : ''}/join/{orgSlug}</span>
        </p>
      </footer>
    </div>
  );
