...
export default function AdminOrganizationsPage() {
  ...
  async function handleSaveEdit() {
    ...
    if (error) {
      setEditError(error.message.includes('Slug') ? 'Slug already taken' : 'Failed to update');
  ...
  return (
    <div className="flex flex-col h-full">
      <TopBar title="Organizations" subtitle={`${data?.total ?? 0} registered`} />
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 text-left">
                  ...
                  <th className="px-4 py-3 font-medium text-gray-500 text-right">Queues</th>
                  <th className="px-4 py-3 font-medium text-gray-500 text-right">Tickets</th>
                  <th className="px-4 py-3 font-medium text-gray-500 text-right">Staff</th>
      ...
      {/* Edit Modal */}
      <Modal open={!!editOrg} onClose={() => setEditOrg(null)} title="Edit Organization">
      ...
      {/* Delete Modal */}
      <Modal open={!!deleteOrg} onClose={() => setDeleteOrg(null)} title="Delete Organization">
        <div className="space-y-4">
          <div className="rounded-lg bg-red-50 border border-red-100 p-4">
            <p className="text-sm text-red-800">
              This will permanently delete <strong>{deleteOrg?.name}</strong> and all its data including:
            </p>
            <ul className="mt-2 text-sm text-red-700 space-y-1">
              <li>- {deleteOrg?.staff_count} staff member{deleteOrg?.staff_count !== 1 ? 's' : ''}</li>
              <li>- {deleteOrg?.queue_count} queue{deleteOrg?.queue_count !== 1 ? 's' : ''}</li>
              <li>- {deleteOrg?.ticket_count} ticket{deleteOrg?.ticket_count !== 1 ? 's' : ''}</li>
